...
class AccessService:
    ...
    async def _check_already_attended(self, student_id: int, session_id: int) -> bool:
        """Return True if this student already has a granted access log for this session."""
        from models.access_log import AccessAction
        query = select(func.count()).where(
            and_(
                AccessLog.student_id == student_id,
                AccessLog.session_id == session_id,
                AccessLog.Action == AccessAction.GRANT,
            )
        )
        result = await self.db.execute(query)
    ...
    async def _mark_attendance(self, student_id: int, session_id: int, marked_by: int):
        """Record attendance in attendance_registers table."""
        from models.attendence_register import AttendanceRegister, AttendanceStatus
