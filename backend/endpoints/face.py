...
async def verify_student(
    ...
    attempt_kwargs: dict = {"terminal_id": terminal_id}
    if exam_session_id:
    ...
    if student_number:
        ...
        res = await db.execute(
            select(Student)
            .options(selectinload(Student.biometric_profile))
            .where(
                Student.StudentNumber == student_number.upper(),
        ...
        s_id = student.id
        s_number = student.StudentNumber
        ...
        res = await db.execute(
            ...
            .join(Student)
            .where(Student.EnrollmentStatus == "Active")
        )
        rows = res.all()
        ...
        res2 = await db.execute(
            select(Student).where(Student.StudentNumber == match.student_number)
        )
        ...
        s_id = matched.id
        s_number = matched.StudentNumber
        s_name = matched.FullName
        attempt_kwargs["student_id"] = s_id
...
async def enroll_face(
    ...
    res = await db.execute(
        ...
        .options(selectinload(Student.biometric_profile))
        .where(
            Student.StudentNumber == student_number.upper().strip(),
            Student.EnrollmentStatus == "Active",
        )
    )
    student = res.scalar_one_or_none()
    ...
    student_number_out = student.StudentNumber
...
async def override_attempt(
    ...
    overridden_by_name = current_user.FullName
